 ...
 * team
 * 
 * @author Loris Previtali Date: April 3, 2017
 */

// Notes for point 2 of assignment: There are no markers on the map
...
public class EarthquakeCityMap extends PApplet {
	...
	private boolean isLand(PointFeature earthquake) {
		...
		// any of them
		// loop for every country, check if the earthquake is in country and return true
	...
	private void printQuakes() {
		...
		for (Marker country : countryMarkers) {
			...
			for (Marker quakes : quakeMarkers) {
				//System.out.println(quakes.getProperties());
				if (quakes instanceof LandQuakeMarker) {
					if (quakes.getStringProperty("country").equals(country.getProperty("name"))) {
						landCount++;
					}
				}
			}
			if(landCount > 0){
				System.out.println(country.getProperty("name") + ":" + landCount);
			}
		}
		int oceanCount = 0;
		for (Marker quakes : quakeMarkers) {
