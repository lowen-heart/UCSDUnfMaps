 ...
 * team
 * 
 * @author Loris Previtali Date: April 6, 2017
 */
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseClicked() {
		...
		// First check if there is a lastClicked marker or not
		// If not (equal to null) determine which marker was selected and
		// hide/display markers
		// if yes de-selects the marker selected before and unhide every marker
	...
	// Helper method when you select a marker
	private void checkForClick() {
		//method variables
		double distance;
		double threatCircle;
		// reference for selected marker by click and unselected
		List<Marker> selectedTypeMarkers;
		List<Marker> unselectedTypeMarkers;
		boolean earthquake;
		
		// select path if lastCLicked is an instance of EarthquakeMarker or CityMarker
		// if earthquake assign quakeMarkers to selectedTypeMarkers, cityMarkers to unselectedTypeMarkers and earthquake to true
		// else viceversa
		if (lastClicked instanceof EarthquakeMarker) {
			selectedTypeMarkers = quakeMarkers;
			unselectedTypeMarkers = cityMarkers;
			earthquake = true;
		} else {
			selectedTypeMarkers = cityMarkers;
			unselectedTypeMarkers = quakeMarkers;
			earthquake = false;
		}
		//hide every marker of the selected type that was not clicked
		for (Marker marker : selectedTypeMarkers) {
			if (marker != lastClicked) {
				marker.setHidden(true);
			}
		}
		// calculate the distance between the clicked marker and the current marker
		// if that is an earthquake jut calculate the threatCircle with the lastClicked
		// else calculate threatCircle for each marker
		// if the distance from marker selected and lastClicked is greater than threatCircle --> hide
		for (Marker marker : unselectedTypeMarkers) {
			distance = marker.getDistanceTo(lastClicked.getLocation());
			if (earthquake) {
				threatCircle = ((EarthquakeMarker) lastClicked).threatCircle();
			} else {
				threatCircle = ((EarthquakeMarker) marker).threatCircle();
			}
